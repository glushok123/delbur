...
PHP_CLI_SERVICE=delbur-php-cli
NODE_CLI_SERVICE=delbur-node-cli
MARIA_DB_CONTAINER=delbur-mariadb
SPHINX_CONTAINER=delbur-sphinx
...
docker-build: \
	docker-build-delbur-nginx \
	docker-build-delbur-php-fpm \
	docker-build-delbur-php-cli

docker-build-delbur-nginx:
	@docker build -t delbur-nginx:${IMAGE_TAG} ./docker/${ENV}/nginx

docker-build-delbur-php-fpm:
	@docker build -t delbur-php-fpm:${IMAGE_TAG} ./docker/${ENV}/php-fpm

docker-build-delbur-php-cli:
	@docker build -t delbur-php-cli:${IMAGE_TAG} ./docker/${ENV}/php-cli
