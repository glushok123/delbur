<div id='grid-product'>
    ...
    <div class='row bg-white cart-custom'>
        ...
        <div class='col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2 align-items-center my-auto justify-content-center text-center'>
            <label class='text-success'>Желаемый бюджет: до {{ $item->acceptable_price }} руб.</label>
            <button type="button" class="btn button-sale-custom btn-outline-success order-create-button text-dark" data-bundleid='{{ $item->id }}' style='color:black;'>Отправить отзыв</button>
            <button type="button" class="btn button-sale-custom btn btn-success order-create-button" data-bundleid='{{ $item->id }}'>Предложить услугу</button>
        </div>
    </div>
    <br>
    @endforeach
