<?php

use Illuminate\Support\Facades\Route;

// --------------------------
// Custom Backpack Routes
// --------------------------
// This route file is loaded automatically by Backpack\Base.
// Routes you generate using Backpack\Generators will be placed here.

Route::group([
    'prefix'     => config('backpack.base.route_prefix', 'admin'),
    'middleware' => array_merge(
        (array) config('backpack.base.web_middleware', 'web'),
        (array) config('backpack.base.middleware_key', 'admin')
    ),
    'namespace'  => 'App\Http\Controllers\Admin',
], function () { // custom admin routes
    Route::crud('product', 'ProductCrudController');
    Route::crud('history-user-request', 'HistoryUserRequestCrudController');
    Route::crud('user', 'UserCrudController');
    Route::crud('bundle', 'BundleCrudController');
    Route::crud('sphere', 'SphereCrudController');
    Route::crud('advertising-network', 'AdvertisingNetworkCrudController');
    Route::crud('order', 'OrderCrudController');
    Route::crud('feedback', 'FeedbackCrudController');
    Route::crud('role', 'RoleCrudController');
    Route::crud('permission', 'PermissionCrudController');
    Route::crud('type-boreholes', 'TypeBoreholesCrudController');
    Route::crud('borehole', 'BoreholeCrudController');
    Route::crud('equipment', 'EquipmentCrudController');
    Route::crud('construction-borehole-pipes', 'ConstructionBoreholePipesCrudController');
    Route::crud('material', 'MaterialCrudController');
}); // this should be the absolute last line of this file